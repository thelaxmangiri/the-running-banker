...
async function handleCreatePost(e) {
  ...
  let finalImageUrl = document.getElementById("post-image-url").value.trim();
  
  try {
    if (file) {
      submitBtn.innerHTML = `<div class="spinner-admin border-white border-t-slate-800"></div> Compressing Cover...`;
      const base64Image = await compressImage(file, 1200);
      
      if (isDemoMode) {
        finalImageUrl = base64Image; // In sandbox, use local base64 data url directly
      } else {
        submitBtn.innerHTML = `<div class="spinner-admin border-white border-t-slate-800"></div> Syncing Cover Image...`;
        const uploadResult = await callAppsScript("uploadBlogPhoto", { 
          imageBase64: base64Image, 
          caption: "Blog Cover: " + document.getElementById("post-title").value.trim() 
        });
        
        if (uploadResult && uploadResult.success && uploadResult.imageUrl) {
          finalImageUrl = uploadResult.imageUrl;
        } else {
          throw new Error(uploadResult.error || "Failed to upload cover photo to CDN.");
        }
      }
    }
    ...
    if (isDemoMode) {
      ...
      submitBtn.innerHTML = `<div class="spinner-admin border-white border-t-slate-800"></div> Syncing Entry...`;
    ...
    console.error(error);
    alert(`Failed to complete blog draft: ${error.message}`);
  } finally {
...
async function handleUploadPhoto(e) {
  ...
  // Resolve progress DOM elements
  const progressContainer = document.getElementById("gallery-progress-container");
  ...
  // Initialize and unhide progress tracking panel
  progressContainer.classList.remove("hidden");
  submitBtn.disabled = true;
  submitBtn.innerHTML = `<div class="spinner-admin border-white border-t-slate-800"></div> Uploading Batch...`;
  
  const baseCaption = document.getElementById("gallery-caption").value.trim();
  const startingOrder = Number(document.getElementById("gallery-order").value) || 10;
  ...
  try {
    for (let i = 0; i < totalFiles; i++) {
      ...
      // Calculate and render progress status before compression
      const progressVal = Math.round((i / totalFiles) * 100);
      progressFill.style.width = `${progressVal}%`;
      progressPercent.textContent = `${progressVal}%`;
      progressStatus.textContent = `Compressing photo ${i + 1} of ${totalFiles} ("${file.name}")...`;
      
      // Compress file down to 1200px width client-side
      const base64Image = await compressImage(file, 1200);
      
      // Resolve custom display order & caption fallback
      const fileOrder = startingOrder + i;
      let fileCaption = baseCaption;
      if (!fileCaption) {
        fileCaption = getFileNameWithoutExtension(file.name);
      } else if (totalFiles > 1) {
        fileCaption = `${baseCaption} (${i + 1})`;
      }
      
      // Render upload state
      progressStatus.textContent = `Uploading photo ${i + 1} of ${totalFiles} to GitHub CDN...`;
      
      if (isDemoMode) {
        // Demonstration Mode In-Memory Injection
        const newPhoto = {
          id: Date.now() + i, // Unique offset
        ...
        if (result && result.success) {
          ...
          console.error(`Failed uploading ${file.name}:`, result ? result.error : "Unknown error");
          failedUploads++;
        }
      }
    }
    
    // Complete visual animation state
    progressFill.style.width = "100%";
    progressPercent.textContent = "100%";
    progressStatus.textContent = `All processes completed! Successfully uploaded ${successfulUploads} of ${totalFiles} photos.`;
    
    // Render sandbox table updates instantly
    if (isDemoMode) {
      localStorage.setItem("trb_mock_gallery", JSON.stringify(galleryPhotos));
      renderGalleryTable(galleryPhotos);
    }
    
    // Quick pause for visual satisfaction before reset
    ...
    if (failedUploads > 0) {
      alert(`Batch uploads finished. ${successfulUploads} succeeded, ${failedUploads} failed. Review script logs for issues.`);
    } else {
      alert(`Batch upload completed! Successfully loaded ${successfulUploads} images.`);
    }
    
  } catch (error) {
    console.error(error);
    alert(`Batch execution error: ${error.message}`);
  } finally {
    progressContainer.classList.add("hidden");
    submitBtn.disabled = false;
    submitBtn.innerHTML = `<i class="fa-solid fa-cloud-arrow-up"></i> Compress &amp; Batch Upload (via GitHub)`;
  }
}
...
async function handleAdminStravaSync() {
  ...
  if (isDemoMode) {
    ...
    if (result && result.success) {
      alert(`Strava webhook pipelines triggered. Added ${result.newActivities} new runs!`);
      loadAllAdminData();
    } else {
...
async function handleAdminSyncAllActivities() {
  const syncBtn = document.getElementById("admin-sync-all-btn");
  const syncIcon = document.getElementById("admin-sync-all-icon");
  const statusContainer = document.getElementById("sync-all-status");
  const statusText = document.getElementById("sync-all-status-text");
  const progressBar = document.getElementById("sync-all-progress-bar");
  const resultContainer = document.getElementById("sync-all-result");
  const resultText = document.getElementById("sync-all-result-text");
  
  if (!confirm("This will fetch your entire Strava history. It may take a minute and consume API rate limits. Continue?")) {
    return;
  }
  ...
  // Initial fake progress to show it's working
  statusText.textContent = "Connecting to Strava API...";
  ...
  if (isDemoMode) {
    ...
    fakeProgressInterval = setInterval(() => {
      progress += 5;
      if (progress > 90) clearInterval(fakeProgressInterval);
      progressBar.style.width = `${Math.min(progress, 90)}%`;
      statusText.textContent = `Paginating through history... (${progress}%)`;
    }, 400);
    ...
    setTimeout(() => {
      ...
      resultText.textContent = "Demo Mode: Synced 42 activities across 3 pages!";
      resultContainer.classList.remove("hidden");
    }, 3000);
  } else {
    // Start real API call. The backend script handles pagination synchronously.
    // We'll update the progress bar vaguely while we wait.
    let progress = 10;
    fakeProgressInterval = setInterval(() => {
      progress += 2;
      // Cap fake progress at 95% while waiting for backend
      if (progress > 95) progress = 95; 
      progressBar.style.width = `${progress}%`;
      
      if (progress < 40) statusText.textContent = "Fetching early activity pages...";
      else if (progress < 70) statusText.textContent = "Still fetching. Hang tight...";
      else statusText.textContent = "Processing and deduplicating records...";
    }, 1000);
    
    try {
      const result = await callAppsScript("syncAllActivities");
      ...
      if (result && result.success) {
        resultText.textContent = `Success! Fetched ${result.totalFetched} runs across ${result.pages} pages. Added ${result.newActivities} new records.`;
        resultText.className = "text-xs font-semibold text-green-700 text-center";
        resultContainer.className = "mt-2 bg-green-50 border border-green-200 rounded-xl p-3";
        resultContainer.classList.remove("hidden");
        loadAllAdminData();
      } else {
        resultText.textContent = `Sync failed: ${result.error || "Unknown error"}`;
        resultText.className = "text-xs font-semibold text-red-700 text-center";
        resultContainer.className = "mt-2 bg-red-50 border border-red-200 rounded-xl p-3";
        resultContainer.classList.remove("hidden");
      }
    } catch (e) {
      clearInterval(fakeProgressInterval);
      ...
      resultText.textContent = `Error connecting to backend: ${e.message}`;
      resultText.className = "text-xs font-semibold text-red-700 text-center";
      resultContainer.className = "mt-2 bg-red-50 border border-red-200 rounded-xl p-3";
      resultContainer.classList.remove("hidden");
    }
...
async function handleSetupDailyTrigger() {
  const btn = document.getElementById("admin-trigger-btn");
  const icon = document.getElementById("admin-trigger-icon");
  
  if (!confirm("This will clear any old Strava sync triggers and create a new daily automated trigger to run every morning at 8:00 AM (based on your Apps Script timezone NPT). Continue?")) {
    return;
  }
  ...
  if (isDemoMode) {
    setTimeout(() => {
      ...
      btn.disabled = false;
      alert("Demo Mode: Daily trigger logic executed successfully!");
    }, 1000);
  } else {
    try {
      const result = await callAppsScript("setupDailyTrigger");
      icon.classList.remove("fa-spin");
      btn.disabled = false;
      
      if (result && result.success) {
        alert("Success! Your automated daily trigger is now active. Background runs will happen daily between 8:00 AM and 9:00 AM.");
      } else {
        alert("Trigger setup failed: " + (result.error || "Unknown error"));
      }
    } catch (e) {
      icon.classList.remove("fa-spin");
      btn.disabled = false;
