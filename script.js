...
async function callAppsScript(action, payload = {}) {
  // If user has not replaced the template deployment ID, fall back to mock data
  ...
  try {
    const response = await fetch(APPS_SCRIPT_URL, {
      method: "POST",
      headers: {
        "Content-Type": "text/plain" // Avoids triggering OPTIONS preflight check
      },
...
function renderMetrics(metrics) {
  ...
  if (metrics.lastRun) {
    ...
    liveTime.textContent = metrics.lastRun.dateString || "Recently synced";
  } else {
    liveLoader.textContent = "No recent activities found on Strava.";
  }

  // Render Target Countdown
  if (metrics.countdown) {
    ...
    document.getElementById("countdown-date").textContent = metrics.countdown.dateString || "Date unconfigured";
  }

  // Render Weekly Volume Target
  ...
  monthlyChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: distances,
        backgroundColor: '#bef264', // Accent neon color
...
function closeLightbox(event) {
  // Close only if background clicked, or X button clicked
    ...
    card.innerHTML = `
      <div>
        <div class="flex items-center justify-between gap-4 mb-4">
          ...
          <span class="text-xs font-semibold text-slate-400 font-mono">${post.created_at || ""}</span>
        </div>
...
function openBlogModal(post) {
  ...
  document.getElementById("modal-post-date").textContent = `Published on ${post.created_at || "Unspecified date"}`;
  
  const categoryBadge = document.getElementById("modal-post-category");
  categoryBadge.textContent = post.category || "General";
  
  // Style category badge accordingly
...
async function syncStrava() {
  ...
  if (result && result.success) {
    ...
    loadAllData(); // Reload stats dynamically
  } else {
...
async function loadAllData() {
  ...
  if (data) {
    ...
    // 3. Render Gallery Library
    renderGallery(data.gallery);
    
    // 4. Render Blog Posts Grid
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Close Mobile Navigation menu when click anchor link
  ...
  // Global Keyboard Control for Modals/Lightboxes
